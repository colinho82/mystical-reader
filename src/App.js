...
async function callOracle(deck, session, images, demo=false, cardNames={}){
  ...
  const cardLines = deck.cards.map((c, i) => {
    ...
    // No name entered and no demo — ask Claude to suggest based on deck type
    return `${c.label} (${c.role}):\n  Card Name: [Reader drew a card but did not type the name — suggest an appropriate ${deck.name} card for this role]\n  Role Purpose: ${c.desc}`;
  }).join("\n\n");
...
{"cards":[{"cardName":"exact name from reader or suggested name","keywords":["word1","word2","word3","word4"],"meaning":"2-3 sentences interpreting this specific card for the question. Reference the card name explicitly. Be personal and specific to their situation.","error":false,"errorReason":""}],"suggestedAnswer":"3-4 flowing paragraphs weaving ALL card names together into one unified answer to the question. Name each card explicitly. Open with the most important insight. Close with one empowering action the client can take today. Warm, honest, mystical tone."}`;
  ...
  if (resData.error) throw new Error(`FUNC_ERROR: ${resData.error}`);
...
function DeckScreen({session,deckIdx,onUpdate,onNext,onGoTo,onClose}){
  ...
  async function read(demo=false){
    ...
    try{
      const r=await callOracle(deck,session,images,demo,cardNames);
      ...
      <div style={{padding:"18px 16px 48px",flex:1}}>
        ...
        {phase==="upload"&&(
          <div>
            ...
            {deck.cards.map(card=>(
              <UploadSlot key={card.id} card={card} value={images[card.id]} deckLight={deck.light} deckColor={deck.color}
                cardName={cardNames[card.id]||""}
                onChangeName={name=>setCardNames(p=>({...p,[card.id]:name}))}
                onChange={img=>setImages(p=>({...p,[card.id]:img}))}/>
            ))}
            <div style={{display:"flex",flexDirection:"column",gap:10,alignItems:"center",marginTop:20}}>
              {/* Allow reading if all cards named OR all images uploaded */}
              {(()=>{
                const namesEntered=deck.cards.every(c=>(cardNames[c.id]||"").trim().length>0);
                const ready=allUp||namesEntered;
                return(
                  <>
                    <Btn onClick={()=>read(false)} disabled={!ready} full>
                      {ready?`✦ Interpret the ${deck.subtitle} Cards`:`Upload cards OR type all card names to continue`}
                    </Btn>
                    {!ready&&(
