...
function Stars(){
  return(
    <div style={{position:"fixed",inset:0,pointerEvents:"none",zIndex:0}}>
      ...
      <style>{`
        @keyframes tw{from{opacity:0.06}to{opacity:0.88}}
        @keyframes float{0%,100%{transform:translateY(0)}50%{transform:translateY(-8px)}}
        @keyframes glow{0%,100%{box-shadow:0 0 14px 3px rgba(201,168,76,0.28)}50%{box-shadow:0 0 28px 8px rgba(201,168,76,0.55)}}
        ...
        .hg:hover{background:rgba(201,168,76,0.12)!important}
      `}</style>
    </div>
  );
}
    ...
    <div style={{marginBottom:16,animation:"up 0.3s ease both"}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
        <div style={{width:27,height:27,borderRadius:"50%",
          background:value?`${deckColor}cc`:"rgba(201,168,76,0.12)",
          border:`1.5px solid ${value?deckLight:"rgba(201,168,76,0.28)"}`,
      ...
      {value?(
        <div style={{display:"flex",gap:12,alignItems:"flex-start",animation:"fi 0.3s ease"}}>
          <div style={{position:"relative",flexShrink:0}}>
            ...
            <div style={{position:"absolute",bottom:-5,left:"50%",transform:"translateX(-50%)",
              background:deckColor,border:`1px solid ${deckLight}77`,borderRadius:10,padding:"2px 8px",
              color:deckLight,fontSize:9,fontWeight:700,whiteSpace:"nowrap"}}>✓ Uploaded</div>
          </div>
          <div style={{display:"flex",flexDirection:"column",gap:7,paddingTop:2,flex:1}}>
            ...
            <label htmlFor={`rc-${uid}`} style={{background:"rgba(201,168,76,0.09)",border:"1px solid rgba(201,168,76,0.3)",borderRadius:20,padding:"7px 13px",cursor:"pointer",color:CREAM,fontSize:11,display:"flex",alignItems:"center",gap:6,userSelect:"none"}}>
              📷 Retake Photo
            </label>
            <input id={`rc-${uid}`} type="file" accept="image/*" capture="environment" style={{display:"none"}} onChange={handleFile}/>
            <label htmlFor={`rg-${uid}`} style={{background:"rgba(201,168,76,0.09)",border:"1px solid rgba(201,168,76,0.3)",borderRadius:20,padding:"7px 13px",cursor:"pointer",color:CREAM,fontSize:11,display:"flex",alignItems:"center",gap:6,userSelect:"none"}}>
              🖼 Choose Gallery
            </label>
            <input id={`rg-${uid}`} type="file" accept="image/*" style={{display:"none"}} onChange={handleFile}/>
            <button onClick={()=>onChange(null)} style={{background:"rgba(180,30,30,0.14)",border:"1px solid rgba(255,80,80,0.26)",borderRadius:20,padding:"7px 13px",cursor:"pointer",color:"#ff9090",fontSize:11,display:"flex",alignItems:"center",gap:6,fontFamily:"inherit"}}>
              🗑 Remove
            </button>
          </div>
        </div>
        ...
        /* Empty state */
        !open?(
          <div onClick={()=>setOpen(true)} style={{height:94,borderRadius:12,border:"2px dashed rgba(201,168,76,0.28)",
            display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",
            cursor:"pointer",background:"rgba(201,168,76,0.03)",transition:"all 0.2s",gap:5}}>
            <div style={{fontSize:24}}>📷</div>
            <div style={{color:CREAM,fontSize:12,opacity:0.55}}>Tap to upload {card.label}</div>
          </div>
        ):(
          <div style={{background:"rgba(12,4,28,0.97)",border:"1px solid rgba(201,168,76,0.35)",borderRadius:12,padding:14,animation:"fi 0.2s ease"}}>
            <div style={{color:GOLD,fontSize:12,fontWeight:700,marginBottom:11}}>How would you like to add {card.label}?</div>
...
function CardBlock({cd,cardMeta,deckLight,deckColor,idx}){
  ...
  return(
    <div style={{
      background:isError?"rgba(180,30,30,0.1)":`linear-gradient(135deg,${deckColor}88,rgba(20,8,40,0.92))`,
      border:isError?"1.5px solid rgba(255,80,80,0.5)":`1.5px solid ${deckLight}44`,
      ...
      {isError?(
        ...
        <div>
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}>
            ...
            <div>
              <div style={{color:"#ff9090",fontSize:16,fontWeight:700,fontFamily:"Palatino Linotype,serif"}}>
                {cardMeta?.label} — Could Not Be Read
              </div>
              <div style={{color:"#ffaa88",fontSize:11,marginTop:2}}>{cardMeta?.role}</div>
            </div>
          </div>
          <div style={{background:"rgba(255,80,80,0.08)",border:"1px solid rgba(255,80,80,0.2)",borderRadius:9,padding:"10px 13px",marginBottom:10}}>
            <div style={{color:"#ff9090",fontSize:12,fontWeight:700,marginBottom:4}}>Problem Detected:</div>
            <div style={{color:CREAM,fontSize:12,lineHeight:1.6,opacity:0.85}}>
              {cd.errorReason||"The card image was unclear, too dark, or the card name could not be identified."}
            </div>
          </div>
          <div style={{color:"#ffcc88",fontSize:12,lineHeight:1.6}}>
            👉 Please reset this step and re-upload a clearer photo of <strong style={{color:"#ffaa44"}}>{cardMeta?.label}</strong>. Make sure the card name is clearly visible, well-lit and in focus.
          </div>
        </div>
      ):(
        /* ── SUCCESS STATE ── */
        <div>
          {/* Card number badge + role */}
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14}}>
            <div style={{width:34,height:34,borderRadius:"50%",background:`${deckColor}cc`,
              border:`2px solid ${deckLight}`,display:"flex",alignItems:"center",justifyContent:"center",
...
function SuggestedAnswer({answer,deck}){
  ...
  return(
    <div style={{
      background:`linear-gradient(135deg,${deck.color}cc,rgba(20,8,40,0.96))`,
      border:`2px solid ${deck.light}55`,borderRadius:16,padding:"20px 18px",
      boxShadow:`0 8px 32px ${deck.color}66`
    }}>
      <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:16}}>
        <span style={{fontSize:26}}>{deck.emoji}</span>
        <div>
          <div style={{color:GOLD,fontSize:16,fontWeight:700,fontFamily:"Palatino Linotype,serif"}}>
            Suggested Reading Answer
          </div>
          <div style={{color:CREAM,fontSize:11,opacity:0.6,marginTop:2}}>
            Based on all {deck.cards.length} cards drawn for your {deck.subtitle.toLowerCase()} reading
          </div>
        </div>
      </div>
...
async function callOracle(deck,session,images,demo=false){
  ...
  const lines=deck.cards.map((c,i)=>{
    ...
    return `Card ${i+1} — ${c.label} (${c.role}):\n  Image status: ${hasImg?"Uploaded — identify the actual card name from the image":"No image — invent a fitting mystical card name for this role"}\n  Role purpose: ${c.desc}`;
  }).join("\n\n");
...
function DeckScreen({session,deckIdx,onUpdate,onNext,onGoTo,onClose}){
  ...
  async function read(demo=false){
    ...
    try{
      ...
      if(!r||!r.cards)throw new Error("empty");
      setCards(r.cards);
      ...
      <div style={{padding:"18px 16px 44px",flex:1}}>
        ...
        {phase==="upload"&&(
          <div>
            ...
            {deck.cards.map(card=>(
              <UploadSlot key={card.id} card={card} value={images[card.id]}
                deckLight={deck.light} deckColor={deck.color}
                onChange={img=>setImages(p=>({...p,[card.id]:img}))}/>
            ))}
                  ...
                  {deck.cards.map((card,i)=>images[card.id]?(
                    <div key={card.id} style={{textAlign:"center",position:"relative"}}>
                      <img src={images[card.id]} alt={card.role} style={{
                        ...
                        border:`2px solid ${cards[i]?.error?"#ff6b6b":deck.light+"55"}`,
    ...
    <div style={{padding:"20px 16px",animation:"up 0.4s ease"}}>
      ...
      <div style={{marginBottom:18}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:7}}>
          {LIFE_AREAS.map(a=>(
            <button key={a.id} className="ha" onClick={()=>{setArea(a.id);setErr("");}} style={{background:area===a.id?`${a.color}cc`:`${a.color}44`,border:`1.5px solid ${area===a.id?GOLD:`${a.color}77`}`,borderRadius:11,padding:"11px 8px",cursor:"pointer",display:"flex",alignItems:"center",gap:7,transition:"all 0.2s",boxShadow:area===a.id?`0 3px 14px ${a.color}55`:"none"}}>
              <span style={{fontSize:18}}>{a.emoji}</span>
              <span style={{color:area===a.id?GOLD:CREAM,fontSize:11,fontFamily:"Palatino Linotype,serif",fontWeight:area===a.id?700:400,textAlign:"left",lineHeight:1.3}}>{a.label}</span>
            </button>
          ))}
        </div>
      </div>
      <div style={{marginBottom:18}}>
        <Label>✦ STEP 2: Describe Your Question</Label>
        <textarea value={q} onChange={e=>{setQ(e.target.value);setErr("");}}
          placeholder="Describe your situation and question in your own words..."
          style={{width:"100%",minHeight:95,background:"rgba(20,8,40,0.82)",border:`1.5px solid ${q.length>0&&valid?GOLD:"rgba(201,168,76,0.25)"}`,borderRadius:11,padding:13,color:CREAM,fontSize:13,lineHeight:1.6,fontFamily:"Palatino Linotype,serif",resize:"none",outline:"none",boxSizing:"border-box",transition:"border 0.2s"}}/>
